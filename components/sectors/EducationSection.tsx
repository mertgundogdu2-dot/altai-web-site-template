          ...
          {courses.map((c: any, i: number) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
              ...
              <div className="aspect-[16/9] overflow-hidden">
                <img src={c.image || getSectorImage("egitim", i)} alt={c.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
              </div>
              <div className="p-5">
                ...
                {c.duration && <p className="mt-3 text-xs font-semibold px-3 py-1 rounded-full inline-block" style={{ color: pc, background: `${pc}10` }}>{c.duration}</p>}
              </div>
            </motion.div>
          ))}
        </div>
